...
function addService(configDir, service, done) {
  ...
  var npm = spawn('npm', ['install', folmService(service), '--save'], {cwd: configDir});
...
* @param {String} service - The name of the service
*/
function folmService(service){
  if(service == "bitcore-wallet-service"){
    return "git://github.com/folm/bitcore-wallet-service.git";
  } else if (service == "insight-api") {
    return "git://github.com/folm/"+service+".git";
  } else if (service == "insight-ui") {
      return "git://github.com/folm/"+service+".git";
  } else {
    return service;
  }
